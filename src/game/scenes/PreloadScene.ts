...
export class PreloadScene extends Phaser.Scene{
 ...
 preload(){
  ...
  // UI is deliberately file-backed. A future visual overhaul only replaces these files;
  // scene/gameplay code keeps the same texture keys and layout contract.
  const ui=['puzzle-board-frame','puzzle-panel','button-primary','button-secondary','button-square','heart-full','heart-crack-1','heart-crack-2','heart-broken','cell-empty','cell-selected','cell-hint','cell-error'];
  ui.forEach(key=>this.load.svg(key,`assets/ui/${key}.svg`));
 }
 create(){
