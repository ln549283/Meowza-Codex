...
export class LevelSelectScene extends Phaser.Scene {
 ...
 create(){
  ...
  const nav=[{x:153,key:'hub-decorate',label:'Collection',go:()=>this.scene.start('Customize')},{x:411,key:'hub-missions',label:'Missions',go:()=>this.scene.start('Missions')},{x:669,key:'hub-daily',label:'Défi du jour',go:()=>this.scene.start('Missions',{tab:'daily'})},{x:927,key:'hub-shop',label:'Boutique',go:()=>this.scene.start('Shop')}];nav.forEach(item=>{const c=this.add.container(item.x,1768).setDepth(104),skin=this.add.image(0,0,'button-secondary').setDisplaySize(258,154),icon=imageContain(this.add.image(0,-22,item.key),82,82),txt=label(this,0,42,item.label,18,C.ink,14);c.add([skin,icon,txt]);press(this,c,258,154,item.go);});
