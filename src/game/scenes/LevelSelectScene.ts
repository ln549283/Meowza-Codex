...
export class LevelSelectScene extends Phaser.Scene {
 ...
 private world!:Phaser.GameObjects.Container;private mist!:Phaser.GameObjects.Container;
 ...
 create(){
  ...
  this.mist=this.add.container(0,0,[
   this.add.rectangle(540,-1800,1080,3400,0xfff8ed,.97),
   imageContain(this.add.image(540,-110,'tree-cloud'),1500,420),
  ]);this.world.add(this.mist);
  ...
  this.status=label(this,540,1560,'',27,C.ink,0).setDepth(105);
  this.locate=this.add.container(540,1590).setDepth(105);
  ...
  this.tap(this.locate,320,90,()=>{this.offset=treeFocus(this.current);this.velocity=0;this.refreshRows();});
  const navY=1740,navWidth=984,left=48,itemW=246;
  this.add.graphics().fillStyle(0xfffaf3,.99).fillRoundedRect(left,1654,navWidth,174,36).lineStyle(2,0xe1d1c0).strokeRoundedRect(left,1654,navWidth,174,36).setDepth(102);
  ...
  this.events.once('shutdown',()=>{this.input.removeAllListeners();this.rows.clear();this.chunks.clear();mask.destroy();clip.destroy();this.registry.set('mapDragging',false);});
  this.refreshRows();
 }
 ...
 private refreshRows(){
  const visible=visibleChunks(this.offset,this.current);
  ...
  for(const i of visible){let c=this.chunks.get(i);const chunk=chunkForIndex(i);if(!c){
   c=this.add.container(CHUNK_X,0);c.add(this.add.image(0,0,`tree-chunk-${chunk.kind}`).setOrigin(0).setScale(CHUNK_SCALE));
   // A quiet secondary accent on B, physically resting on the shelf.
   if(chunk.kind==='b')c.add(imageContain(this.add.image(827*CHUNK_SCALE,116*CHUNK_SCALE,'tree-yarn'),50,50));
   this.world.addAt(c,0);this.chunks.set(i,c);
  }c.y=1040+chunk.top+this.offset;}
  const levels=visibleTreeLevels(this.current,this.offset);
  for(const [n,row]of this.rows)if(!levels.includes(n)){row.destroy();this.rows.delete(n);}
  for(const n of levels){let row=this.rows.get(n);if(!row){row=this.makeRow(n);this.world.add(row);this.rows.set(n,row);}row.y=treeY(n,this.offset);if(row.input)row.input.enabled=row.y>TOP+50&&row.y<BOTTOM-50;}
  this.world.bringToTop(this.mist);this.mist.y=treeY(this.current,this.offset)-240;
  if(this.locate)this.locate.setVisible(treeY(this.current,this.offset)<TOP+70||treeY(this.current,this.offset)>BOTTOM-140);
 }
 private makeRow(n:number){
  ...
  this.tap(row,230,112,()=>{if(row.y>TOP+50&&row.y<BOTTOM-50)void this.play(n);});
  return row;
 }
