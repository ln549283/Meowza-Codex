...
export class LevelSelectScene extends Phaser.Scene {
 ...
 private refreshRows(){const visible=visibleTreeLevels(this.current,this.offset);for(const [n,row] of this.rows)if(!visible.includes(n)){row.destroy();this.rows.delete(n);}for(const n of visible){let row=this.rows.get(n);if(!row){row=this.makeRow(n);this.rows.set(n,row);}const y=treeY(n,this.offset);row.y=y;row.setAlpha(y>1570?Phaser.Math.Clamp((1670-y)/100,.08,1):y<210?Phaser.Math.Clamp((y-120)/90,.15,1):1);}this.mist.y=treeY(this.current,this.offset)-260;}
 private makeRow(n:number){
  const row=this.add.container(0,0).setDepth(10),x=[360,690,405,675,355,705,415,665][(n-1)%8]!,progress=SaveService.data.progress[journeyId(n)],done=!!progress?.completed,spec=journeySpec(n),add=<T extends Phaser.GameObjects.GameObject>(o:T)=>{row.add(o);return o;};
  if(n>1)add(imageContain(this.add.image(540,TREE_STEP/2+18,'tree-post-long'),90,TREE_STEP+76));else{add(imageContain(this.add.image(540,138,'tree-post-short'),90,290));add(imageContain(this.add.image(540,254,'tree-base'),390,176));}
  const beamMid=(540+x)/2,beamWidth=Math.abs(540-x)+54;add(imageContain(this.add.image(beamMid,64,'tree-hammock-bar'),beamWidth,44));add(imageContain(this.add.image(540,64,'tree-junction-t'),72,72));add(imageContain(this.add.image(x,64,'tree-junction-round'),54,54));
  ...
  if(n%10===0){
   const unlocked=SaveService.trailCompletedCount()>=n,habitat=habitatStyleForLevel(n);attachSide(habitat.texture,108,habitat.w,habitat.h,unlocked?1:.24);const catId=SaveService.data.refuges[String(n)],cat=catId?catById(catId):undefined;
   if(unlocked){
    if(cat)add(imageContain(this.add.image(side,92+habitat.catY,cat.texture),166,166).setDepth(4));
    else{add(this.add.image(side,90,'ui-circle').setDisplaySize(68,68).setAlpha(.8));add(label(this,side,90,'+',36,C.ink,18));}
    const hit=this.add.container(side,108).setDepth(8),selected=catId?Math.max(0,collectionCats.findIndex(c=>c.id===catId)):0;add(hit);press(this,hit,250,188,()=>{if(this.registry.get('mapDragging'))return;this.scene.start('Customize',{tab:3,selected,targetLevel:n});});
   }else add(imageContain(this.add.image(side,94,'ui-lock'),48,48).setAlpha(.64).setDepth(5));
  }else if(n%3===0){
   const decoX=x<540?610:470,key=n%9===0?'tree-hanging-plant':n%6===0?'tree-plant':'tree-yarn',w=key==='tree-yarn'?48:68,h=key==='tree-yarn'?48:86;add(imageContain(this.add.image(decoX,130,key),w,h).setAlpha(.68));
  }
  ...
  if(done){const stars=progress.bestErrors===0?3:progress.bestErrors===1?2:1;add(imageContain(this.add.image(x,30,`stars-${stars}`),164,60));}
  return row;
 }
}
