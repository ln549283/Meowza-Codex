...
export class LevelSelectScene extends Phaser.Scene {
 ...
 create(){
  ...
  this.current=nextSummit(SaveService.data.progress);const current=this.current;this.offset=this.pendingOffset??treeFocus(current);this.mist=this.add.container(0,0,[imageContain(this.add.image(540,0,'tree-cloud'),820,340)]).setDepth(25);this.refreshRows();
 ...
 private refreshRows(){const visible=visibleTreeLevels(this.current,this.offset);for(const [n,row] of this.rows)if(!visible.includes(n)){row.destroy();this.rows.delete(n);}for(const n of visible){let row=this.rows.get(n);if(!row){row=this.makeRow(n);this.rows.set(n,row);}row.y=treeY(n,this.offset);}this.mist.y=treeY(this.current,this.offset)-330;}
 private makeRow(n:number){
  ...
  if(n%10===0){
   ...
   add(imageContain(this.add.image(side,126,n%12===0?'tree-hanging-plant':'tree-plant'),94,118).setAlpha(.88));
  }else if(n%7===0){
   add(imageContain(this.add.image(side,122,'tree-yarn'),72,72).setAlpha(.86));
  }
