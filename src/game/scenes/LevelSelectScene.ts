...
export class LevelSelectScene extends Phaser.Scene {
 ...
 create(){
  ...
  const nav=[{x:135,key:'hub-decorate',label:'Collection',go:()=>this.scene.start('Customize')},{x:405,key:'hub-missions',label:'Missions',go:()=>this.scene.start('Missions')},{x:675,key:'hub-daily',label:'Défi du jour',go:()=>this.scene.start('Missions',{tab:'daily'})},{x:945,key:'hub-shop',label:'Boutique',go:()=>this.scene.start('Shop')}];nav.forEach(item=>{const c=this.add.container(item.x,1800).setDepth(104),skin=this.add.image(0,0,'button-secondary').setDisplaySize(270,154),icon=imageContain(this.add.image(0,-24,item.key),86,86),txt=label(this,0,50,item.label,19,C.ink,15);c.add([skin,icon,txt]);press(this,c,270,154,item.go);});
