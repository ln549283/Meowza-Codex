...
export class LevelSelectScene extends Phaser.Scene {
 ...
 create(){
  ...
  group.forEach((level,i)=>{
   const x=540+Math.sin(i*1.45)*235,y=base-i*250,unlocked=SaveService.isUnlocked(level.id),progress=SaveService.data.progress[level.id];if(!unlocked){cloud(this,x,y,360);return;}
  ...
  label(this,540,base+195,config.subtitle,34);panel(this,540,130,1040,245,0xfff9f4,.97).setScrollFactor(0).setDepth(100);roundButton(this,95,102,'‹',()=>this.scene.start('Home')).setScrollFactor(0).setDepth(101);label(this,540,90,'L’arbre des petits bonheurs',44).setScrollFactor(0).setDepth(101);label(this,540,157,`${SaveService.completedCount(this.chapter)} / ${group.length}  ·  ${config.subtitle}`,27).setScrollFactor(0).setDepth(101);
  chapters.forEach((ch,i)=>button(this,150+i*260,284,242,ch.name,()=>this.scene.restart({chapter:ch.id}),ch.id===this.chapter?ch.color:0x9b8ca8).setScale(.92).setScrollFactor(0).setDepth(101));panel(this,540,1800,1040,245,0xfff9f4,.97).setScrollFactor(0).setDepth(100);
  const available=group.some(l=>SaveService.isUnlocked(l.id));const unlockText=this.chapter==='medium'?'Termine 10 niveaux Facile pour accéder aux cabanes.':this.chapter==='hard'?'Termine 15 niveaux Moyen pour atteindre les perchoirs.':'Termine les 30 niveaux Difficile pour percer les nuages.';label(this,540,1727,available?'Fais glisser pour explorer l’arbre':unlockText,available?28:25).setScrollFactor(0).setDepth(101);button(this,540,1830,620,available?'Revenir à mon perchoir':'Retour à mon parcours',()=>{if(available)this.focus(base-group.indexOf(current)*250);else this.scene.restart({chapter:chapters[Math.max(0,chapters.indexOf(config)-1)]!.id});},config.color).setScrollFactor(0).setDepth(101);
  this.focus(base-group.indexOf(current)*250);let previous=0,dragging=false;this.input.on('pointerdown',(p:Phaser.Input.Pointer)=>{this.registry.set('mapDragging',false);previous=p.y;dragging=p.y>355&&p.y<1660;});this.input.on('pointermove',(p:Phaser.Input.Pointer)=>{if(!p.isDown||!dragging)return;if(p.getDistance()>18)this.registry.set('mapDragging',true);this.cameras.main.scrollY=Phaser.Math.Clamp(this.cameras.main.scrollY+previous-p.y,0,height-1920);previous=p.y;});this.input.on('wheel',(_p:unknown,_o:unknown,_x:number,dy:number)=>{this.cameras.main.scrollY=Phaser.Math.Clamp(this.cameras.main.scrollY+dy,0,height-1920);});this.events.once('shutdown',()=>{this.input.removeAllListeners();this.registry.set('mapDragging',false);});
 }
