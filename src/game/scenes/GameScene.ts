...
export class GameScene extends Phaser.Scene {
 ...
 create(){
  ...
  label(this,540,132,difficultyLabel(),22,'#775F68',20);
  ...
  const selectorY=boardY+boardSize/2+100;
  const selectorCards:Phaser.GameObjects.Container[]=[];
  const select=(value:1|2)=>{board.brush=value;selectorCards.forEach((c,i)=>{const selected=i+1===value;const ring=c.list[0] as Phaser.GameObjects.Image;ring.setTexture(selected?(value===1?'tile-lilac':'tile-peach'):'ui-circle');c.setScale(selected?1.06:1).setAlpha(selected?1:.78);});};
  ([1,2] as const).forEach((value,i)=>{const c=this.add.container(395+i*290,selectorY),ring=this.add.image(0,0,'ui-circle').setDisplaySize(170,170),cat=imageContain(this.add.image(0,-5,value===1?'grey-cat':'orange-cat'),135,135),name=label(this,0,92,value===1?'Nimbus':'Moka',21,value===1?'#5f4a70':'#8b4c3d',20);c.add([ring,cat,name]);press(this,c,190,205,()=>{if(this.won){return;}select(value);if(!SaveService.data.settings.reducedMotion)this.tweens.add({targets:cat,angle:value===1?-5:5,duration:90,yoyo:true,ease:'Sine.Out'});});selectorCards.push(c);});select(1);

  const usedHints=new Set<string>(saved?.id===level.id?saved.hintPositions:[]);const footerY=Math.min(1790,selectorY+205);
  const heart=this.add.image(845,footerY-18,'heart-full').setDisplaySize(112,112);
  imageContain(this.add.image(775,footerY+58,'hub-kibble'),40,40);const kibble=label(this,860,footerY+58,'',24,C.ink,0);
