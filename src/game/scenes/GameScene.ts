...
export class GameScene extends Phaser.Scene {
 ...
 create(){
  ...
  const hint=button(this,540,1680,310,'✦ Indice',()=>{if(this.won)return;const found=findHint(board.grid,level);if(found){this.hints++;board.reveal(found.position,found.value);AudioService.play('hint');if(!this.won)status.setText('Une case dévoilée pour te guider.');}else status.setText('Corrige les cases en rose ou annule un coup.');},C.teal);
