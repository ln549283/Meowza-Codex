...
export class CustomizeScene extends Phaser.Scene{
 ...
 create(){
  ...
  const key=tabs[this.tab]!.key,gridTop=338,gridX=20,cell=(1080-gridX*2)/4,totalSlots=16;
  ...
  for(let i=0;i<totalSlots;i++){
   ...
   if(owned&&isActual){
    ...
    imageContain(this.add.image(x,y-10,'ui-lock'),38,38).setAlpha(.42).setDepth(2);label(this,x,y+42,'???',21,'#8f8197',16).setAlpha(.78).setDepth(3);
   }
   cellSkin.setDepth(-1);
  }
  ...
  if(key==='cats'){
   ...
   if(cat&&owned){
    ...
    const sameTarget=!!target&&assigned?.[0]===String(target),canPlace=!!placementLevel,action=this.add.container(855,detailY),skin=this.add.image(0,0,sameTarget?'button-disabled':canPlace?'button-primary':'button-disabled').setDisplaySize(270,92),txt=label(this,0,0,sameTarget?'Retirer':target?'Installer ici':assigned?'Déplacer':'Placer',24,sameTarget||!canPlace?C.ink:'#ffffff',18);action.add([skin,txt]);
    if(sameTarget)press(this,action,270,92,()=>{SaveService.assignCat(target!,null);this.scene.start('LevelSelect');});
    else if(canPlace)press(this,action,270,92,()=>{SaveService.assignCat(placementLevel!,cat.id);this.scene.start('LevelSelect');});
   }else{
    label(this,540,detailY-18,'Les compagnons restent secrets jusqu’à leur découverte.',24,'#78647d',18).setWordWrapWidth(760);label(this,540,detailY+28,target?`Le support du niveau ${target} attend un chat débloqué.`:'Une nouvelle place pour chat se débloque tous les 10 niveaux.',19,'#95869e',16).setWordWrapWidth(820);
   }
  }else{
