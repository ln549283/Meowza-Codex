...
export class LostScene extends Phaser.Scene {
 ...
 create({reason='errors'}:{reason?:string}={}){cozyBackground(this);title(this,'Chat alors…',390,80);imageContain(this.add.image(540,720,'grey-cat'),340,340);drawAttemptHeart(this.add.graphics(),540,960,3,.85);label(this,540,1110,reason==='time'?'Le temps est écoulé. On réessaie ?':'Trois erreurs. Une nouvelle tentative ?',36);label(this,540,1210,'Nouvelle tentative : 3 indices, à partir de 15 croquettes.',30);button(this,540,1430,720,'Recommencer',()=>{SaveService.restartAttempt();this.scene.start('Game');},C.pink);if(GameRegistry.selected?.timed&&(SaveService.data.failures[GameRegistry.selected.id]??0)>=3)button(this,540,1790,720,'Réessayer · temps +50 % offert',()=>{const l=GameRegistry.selected!;SaveService.restartAttempt();SaveService.remember(l.id,l.initial,0,0,(l.timeLimit??360)*1.5,false);this.scene.start('Game');},C.orange);button(this,540,1610,720,'Retour à l’arbre',()=>this.scene.start('LevelSelect'),C.teal);}
}
