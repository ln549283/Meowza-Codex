...
import { LostScene,MissionsScene,ShopScene } from './game/scenes/ExtraScenes';
...
function start(){
 ...
 void App.addListener('backButton',()=>{const active=game.scene.getScenes(true).at(-1);if(!active)return;const key=active.scene.key;if(key==='Hint'||key==='Rules'&&game.registry.get('rulesFromGame')){active.scene.stop();game.scene.resume('Game');}else if(key==='Game')active.scene.start('LevelSelect');else if(key==='Home')void App.minimizeApp();else if(key==='LevelSelect')active.scene.start('Home');else if(key!=='Boot'&&key!=='Preload')active.scene.start('LevelSelect');});
 if(import.meta.env.DEV)Object.assign(window,{meowza:game});
}
try{start();}catch(error){console.error('Meowza startup failed',error);const status=document.getElementById('startup-status');if(status)status.textContent='Le jeu n’a pas pu démarrer. Réessaie.';const retry=document.getElementById('startup-retry');if(retry)retry.hidden=false;}
