...
export const treeLimit=(current:number)=>Math.max(0,(current-1)*TREE_STEP+170);
...
export function visibleTreeLevels(current:number,offset:number){
 const first=Math.max(1,Math.ceil((1080+offset-2180)/TREE_STEP)+1);
 const last=Math.min(current,Math.floor((1080+offset+360)/TREE_STEP)+1);
