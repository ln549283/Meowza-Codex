export const TREE_STEP=470;
export const treeY=(level:number,offset:number)=>1160-(level-1)*TREE_STEP+offset;
export const treeLimit=(current:number)=>Math.max(0,(current-1)*TREE_STEP+190);
...
export function visibleTreeLevels(current:number,offset:number){
 const first=Math.max(1,Math.ceil((1160+offset-2220)/TREE_STEP)+1);
 const last=Math.min(current,Math.floor((1160+offset+320)/TREE_STEP)+1);
