...
it('loads v1 saves, preserves best scores and serializes writes',async()=>{
 ...
 await save.load();assert.equal(save.data.session,null);assert.equal(save.data.settings.sounds,false);await save.complete('easy-01',5,3);assert.equal(save.data.progress['easy-01']?.stars,3);assert.equal(save.data.stats.levelsCompleted,1);save.remember('easy-02',[[0]],0,0);await save.persist();assert.equal(JSON.parse(writes.at(-1)!).session.id,'easy-02');
});
