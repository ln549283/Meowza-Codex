...
it('migrates v1 saves, drops stars, preserves best scores and serializes writes',async()=>{
 const writes:string[]=[];
 ...
 await save.load();assert.equal(save.data.saveVersion,2);assert.equal(save.data.session,null);assert.equal(save.data.settings.sounds,false);assert.deepEqual(save.data.progress['easy-01'],{completed:true,bestErrors:0,bestHints:0});await save.complete('easy-01',5,3);assert.equal(save.data.progress['easy-01']?.bestErrors,0);assert.equal(save.data.stats.levelsCompleted,1);save.remember('easy-02',[[0]],0,0);await save.persist();assert.equal(JSON.parse(writes.at(-1)!).session.id,'easy-02');
});
it('recovers persistence after a transient storage failure',async()=>{
 ...
 await save.complete('easy-01',0,0);assert.equal(save.saveFailed,true);assert.equal(save.data.progress['easy-01']?.completed,true);fail=false;await save.persist();assert.equal(save.saveFailed,false);
});
