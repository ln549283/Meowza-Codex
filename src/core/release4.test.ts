...
test('timed reward, failed attempts and timer survive reload without charging a life',async()=>{
 ...
 s.remember(l.id,l.initial,2,1,123,true,true);await s.persist();const copy=new SaveServiceImpl(storage);await copy.load();assert.equal(copy.data.session?.remaining,123);assert.equal(copy.data.session?.failed,true);copy.restartAttempt();assert.equal(copy.data.kibble,60);await copy.complete(l.id,2,1);assert.equal(copy.data.lastReward,18);assert.equal(copy.data.kibble,78);await copy.complete(l.id,0,0);assert.equal(copy.data.kibble,78);
 assert.deepEqual([0,1,2,3,20].map(hintCost),[15,25,40,null,null]);assert.deepEqual(['easy','medium','hard','extreme'].map(d=>rewardFor(d as 'easy')),[5,8,12,15]);
});
